...
$(document).ready(function () {
  ...
  $('.findbloodstocktype34').select2({
    ...
    setTimeout(function () {
      document.getElementById("findbloodstocktype34").focus();
    }, 200);
  });
...
function clearPatient() {
  ...
  document.getElementById('img_profile').src = "assets/images/profile.png"; // clearFormTab1();
}
  ...
  if (data.patientrh == "Rh-") {
    swal({
      ...
      if (data.group_antibody.replace(/,/g, "") != "") {
        errorSwalAnti("ผู้ป่วยมีประวัติ Antibody ", substring_1(data.group_antibody), data); // alert(data.group_antibody.replace(/,/g, ""));
...
function chActive(id, self, status) {
  for (i = 0; i < count; i++) {
    if (status == 1) {
      if (document.getElementById("trblood" + i)) document.getElementById("trblood" + i).style.background = "#E0E0E0";
    } else {
      if (document.getElementById("trblood" + i)) document.getElementById("trblood" + i).style.background = "#FFF";
    }
  }
...
function chActiveAddRowReqBlood(id, self, bloodstocktypeid) {
  var qty = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
  var vgroupid = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : "";
...
function addRowItemReqCrossmatch() {
  var im = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  var index = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  var event_data = '';
  event_data += '<tr>';
...
function setTableCheckBlood(data) {
  ...
  setABOCellGroupSerum("#a2cells", data.a2cells); // setBloodGroup("#bloodgroup",data.bloodgroup);
  ...
  $("#confirmbloodgroupshow").val(data.confirmbloodgroup); // setRh4("#confirmrhid", data.confirmrhid);
  ...
  document.getElementById('phenotypeLable').innerHTML = data.phenotypedisplay; // เพิ่มบุตร
  ...
  console.log(data.crossmatchuser); // setDataModalSelectValue('crossmatchuserid', data.crossmatchuserid, data.crossmatchuser);
  ...
  document.getElementById('checkbloodremark2').value = data.checkbloodremark2; // document.getElementById('checkpatientdate_show').value = ((data.checkpatientdate == "" || data.checkpatientdate == "0000-00-00") ? "" : getDMY(data.checkpatientdate));
  ...
  document.getElementById('confirmbloodgroupqueuetime').value = data.confirmbloodgroupqueuetime; // var select = $('#checkpatientuser');
  ...
  loadTableCheckBloodHistory(); // End Table ABO
  // Start Table Rh
...
function msgCheckSpecial(item) {
  ...
  if (item.plasmaexchange == "1" || item.screenforplateletantibody == "1" || item.hlacrossmatchsingledonorplatelet == "1" || item.washedredbloodcell == "1" || item.light == "1") {
    var textMsg = "";
...
function checkFindBloodPatient() {
  var page = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 3;
      ...
      success: function success(data) {
        if (data.data.length > 0) {
          setTimeout(function () {
            ...
            console.log("Autologous/Direct"); //your code to be executed after 1 second
          }, 1500);
        } else {
          if (page == 5) {
...
function setPrintName() {
  ...
  if (printers) printers.forEach(function (entry) {
