...
$(document).ready(function () {
  ...
  $('#requestunit').select2({
    ...
    document.getElementById("forchildren").checked = false; // document.getElementById("forchildren").disabled = (!(e.params.args.data.id == 232));
  });
  $('#usedunit').select2({
    allowClear: true,
    width: "100%",
  ...
  $('#bloodnotificationtypeid').select2({
    ...
    if ($('#requestbloodid').val() == '') setOpenBloodBlank(e.params.args.data.item.bloodnotificationtypeid);
  });
  $('#doctorid').select2({
    allowClear: true,
    width: "100%",
    ...
    templateResult: function templateResult(state) {
      ...
      var strState = state.text.split("|");
      var $state = $('<span class="select2-span">&nbsp;' + strState[1] + '&nbsp;</span>' + '<span class="select2-span2">&nbsp;' + strState[2] + '&nbsp;</span>' + ' <span >&nbsp;' + strState[0] + '</span>');
      return $state;
    },
    templateSelection: function templateSelection(state) {
      if (!state.id) {
  ...
  $('#diagnosisid').select2({
    ...
    templateSelection: function templateSelection(state) {
      ...
      var strState = state.text.split("|");
      var $state = $('<span >' + strState[1] + '</span>');
      return $state;
    }
  }).on("select2:selecting", function (e) {
...
function setRequestBloodBloodStockType(self) {
  ...
  if (checkDuplicateTablePlatelet(self.value)) {
    ...
    mgsError("ขออภัยค่ะ!", "มีการเพิ่ม Platelet แล้ว", _callback);
  }

  getTableTypeOfRequestBlood();
}

function checkDuplicateTableReq() {
  ...
  $.each(body.children, function (ind, vl) {
    if (v == 'LPRC' || v == 'LDPRC' || v == 'PRC' || v == 'SDR' || v == 'LDSDR') {
      if (vl.children[2].children[0].value == 'LPRC' || vl.children[2].children[0].value == 'LDPRC' || vl.children[2].children[0].value == 'PRC' || vl.children[2].children[0].value == 'SDR' || vl.children[2].children[0].value == 'LDSDR') {
        if (count > 0) status = true;
        count++;
      }
    } else if (v == 'PC' || v == 'LPPC' || v == 'SDP' || v == 'SDP_PAS' || v == 'LDPPC' || v == 'LDPPC_PAS' || v == 'LPPC_PAS' || v == 'PLDPC') {
      if (vl.children[2].children[0].value == 'PC' || vl.children[2].children[0].value == 'LPPC' || vl.children[2].children[0].value == 'SDP' || vl.children[2].children[0].value == 'SDP_PAS' || vl.children[2].children[0].value == 'LDPPC' || vl.children[2].children[0].value == 'LDPPC_PAS' || vl.children[2].children[0].value == 'LPPC_PAS' || vl.children[2].children[0].value == 'PLDPC') {
        if (count > 0) status = true;
        count++;
      }
    } else if (v == 'FFP' || v == 'CRP') {
      if (vl.children[2].children[0].value == 'FFP' || vl.children[2].children[0].value == 'CRP') {
        if (count > 0) status = true;
        count++;
      }
    } else if (v == 'CRYO' || v == 'HTFDC') {
      if (vl.children[2].children[0].value == 'CRYO' || vl.children[2].children[0].value == 'HTFDC') {
        if (count > 0) status = true;
        count++;
      }
...
function checkDuplicateTablePlatelet() {
  var v = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  var body = document.getElementById("list_table_tab1").getElementsByTagName('tbody')[0];
  var status = false;
  var count = 0;
...
function checkDateRequestBlood() {
  ...
  for (var i = 1; i < rows.length; i++) {
    if (!rows[i].cells[5].children[0].hidden && rows[i].cells[5].children[0].value == "") status = true;
  }
...
function deleteRowReqBlood() {
      ...
      $.ajax({
        ...
        success: function success(data) {
          if (data.status == true) {
            myAlertTopDelete();
            data = {
              grouprequestbloodcancelname: inputValue,
              requestbloodcancelother: '',
  ...
  $('#requestbloodcancel').select2({
    ...
    // tags: [],
    ajax: {
      url: "data/masterdata/request-blood-cancel.php",
      dataType: 'json',
      type: "GET",
      quietMillis: 50,
      data: function data(keyword) {
    ...
    success: function success(resultData) {
      ...
      if (resultData.status) {
        if (data.requestbloodstatusid == '3') {
          ...
          if (data.grouprequestbloodcancelname == undefined) data.grouprequestbloodcancelname = "";
          document.getElementById("labcheckrequestcancelname").innerHTML = 'สาเหตุ : ' + data.grouprequestbloodcancelname + ' , ' + requestbloodcancelname;
        }
...
function clearFormTab1() {
  ...
  $("#group_antibody").val(''); // setDataModalSelectValue('bloodgroupid','','');
...
function removeOthers(self) {
  $('option', self).not(':eq(0), :selected').remove(); //now refresh your select2
}
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (obj.length > 0) {
        ...
        $("#group_antibody").val(removeCommaAni(obj[0].group_antibody_all)); // document.getElementById('requestantibody').innerHTML = obj[0].group_antibody;
...
function setOpenBloodBlank() {
  var bloodnotificationtypeid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
...
function setDurgicalDateLableStar() {
  var bloodnotificationtypeid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
...
function setBloodDrillerIdStar() {
...
function setBloodCertifierLableStar() {// document.getElementById("bloodCertifierLableStar").style.visibility = (status) ? "visible" : "hidden";
...
function checkEntryText() {
  ...
  if (checkOptionText('requestunit')) {
    var callback = function callback(inputValue) {
      document.getElementById("requestunit").focus();
    };
    ...
    var _callback2 = function _callback2(inputValue) {
      document.getElementById("bloodnotificationtypeid").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ ประเภทการแจ้งขอเลือด", _callback2);
    status = true;
    ...
    mgsError("ขออภัยค่ะ!", "กรุณาระบุ วันที่ต้องการใช้เลือด", _callback3);
    status = true;
    ...
    mgsError("ขออภัยค่ะ!", "กรุณาระบุ วันที่ต้องการใช้เลือด", _callback4);
    status = true;
    ...
    mgsError("ขออภัยค่ะ!", "กรุณาระบุ กำหนดวันผ่าตัด", _callback5);
    status = true;
  } else if (checkOptionText('nurseid')) {
    var _callback6 = function _callback6(inputValue) {
      document.getElementById("nurseid").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ พยาบาลผู้รับคำสั่ง", _callback6);
    status = true;
  } else if (checkOptionText('doctorid')) {
    var _callback7 = function _callback7(inputValue) {
      document.getElementById("doctorid").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ แพทย์ผู้สั่ง", _callback7);
    status = true;
  } else if (checkOptionText('blood_driller_id') && document.getElementById("bloodsampletube1").checked) {
    var _callback8 = function _callback8(inputValue) {
      document.getElementById("blood_driller_id").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ ผู้เจาะเลือด", _callback8);
    status = true;
  } else if (checkOptionText('blood_certifier_id') && document.getElementById("bloodsampletube1").checked && requestbloodid == '') {
    var _callback9 = function _callback9(inputValue) {
      document.getElementById("blood_certifier_id").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ ผู้ตรวจสอบ", _callback9);
    status = true;
  } else if (checkOptionText('diagnosisdetail')) {
    var _callback10 = function _callback10(inputValue) {
      document.getElementById("diagnosis").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ Diagnosis", _callback10);
    status = true;
  } else if (!document.getElementById("bloodsampletube1").checked && !document.getElementById("bloodsampletube2").checked) {
    var _callback11 = function _callback11(inputValue) {
      document.getElementById("bloodsampletube1").focus();
    };

    mgsError("ขออภัยค่ะ!", "กรุณาระบุ Blood Sample/Tube", _callback11);
    status = true;
  }
...
function countAdsol() {
  ...
  for (var i = 1; i < rows.length; i++) {
    if (rows[i].cells[1].children[0].checked) countTotal++;
  }

  setCountAdsol(countTotal);
}

function setCountAdsol() {
...
function checkOptionText() {
  var textName = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  var status = false;
