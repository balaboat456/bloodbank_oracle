...
$(document).ready(function () {
  ...
  $("#finfhn").keyup(function (event) {
    if (event.keyCode === 13) {
      ...
      if (tabactive == "1_1") {
        if (!findTableHN1_1(hn)) errorSwal2("ไม่พบ HN นี้", "finfhn");
      } else if (tabactive == "1_4") {
        if (!findTableHN1_4(hn)) errorSwal2("ไม่พบ HN นี้", "finfhn");
      } else if (tabactive == "2_1") {
        if (!findTableHN2_1(hn)) errorSwal2("ไม่พบ HN นี้", "finfhn");
      } else if (tabactive == "2_3") {
        if (!findTableHN2_3(hn)) errorSwal2("ไม่พบ HN นี้", "finfhn");
      }
  ...
  $("#findhn2").keyup(function (event) {
    if (event.keyCode === 13) {
      ...
      if (tabactive == "2_2") {
        ...
        } else if (!findTableHN2_2(hn2)) errorSwal2("ไม่พบ HN นี้", "findhn2");
      }
  ...
  function checkLoadPatient(patient) {
    ...
    }, _defineProperty(_swal, "confirmButtonClass", ""), _defineProperty(_swal, "confirmButtonText", "ตกลง"), _defineProperty(_swal, "closeOnConfirm", true), _defineProperty(_swal, "allowOutsideClick", false), _swal)).then(function (value) {
      if (value) {
        $.ajax({
          type: 'GET',
          dataType: 'json',
...
function getBagNumberHN() {
  var hn = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var donation_get_type_id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  spinnershow();
  $.ajax({
    type: 'GET',
    dataType: 'json',
    ...
    success: function success(data) {
      if (data.data.length > 0) {
        ...
        swal((_swal2 = {
          ...
          // dangerMode: true,
          confirmButtonClass: "btn-success"
        }, _defineProperty(_swal2, "confirmButtonClass", ""), _defineProperty(_swal2, "confirmButtonText", "ตกลง"), _defineProperty(_swal2, "closeOnConfirm", true), _swal2), function (inputValue) {// if (inputValue) {
...
function findTableHN2_1(number) {
  ...
  for (var i = 2; i < rows.length; i++) {
    ...
    if (item[0].hn == number) {
      ...
      indexSinge = i; // item[0].isreadyblood = true;
  ...
  for (var i = 2; i < rows.length; i++) {
    ...
    if (item[0].hn == number) {
      ...
      if (item[0].cm_bloodtype == "CRYO") {
        ...
        row.cells[12].children[0].disabled = false;
        item[0].ispayblood = true;
        row.cells[0].innerHTML = JSON.stringify(item); // row.cells[13].children[0].required = true;
...
function findTableBagNumber2_2(number) {
  ...
  for (var i = 2; i < rows.length; i++) {
    ...
    if (item[0].cm_bag_number == number && item[0].cm_bloodtype == bloodtype && item[0].hn == hnfind2) {
      ...
      rows[i].cells[12].getElementsByTagName('input')[0].checked = true;
      item[0].ispayblood = true;
      row.cells[0].innerHTML = JSON.stringify(item); // ปิดการใช้งานช่องกระติกไปก่อน 
      // row.cells[13].children[0].required = true;

      console.log(item[0].bloodstocktypegroupid);

      if (item[0].bloodstocktypegroupid == 1) {// ปิดการใช้งานช่องกระติกไปก่อน 
        // row.cells[14].children[0].required = true;
      }
  ...
  if (index !== "") {
    if (checkPayExp(arrayCheckPay.bloodexp, arrayCheckPay.hn, arrayCheckPay.bloodstocktypegroupid, index)) {
      swal({
...
function loadTableAll() {
  console.log("****2*****");
  if (bloodQueueCheckState) loadTableBloodQueue();
  loadTableBloodQueueCancel();
  loadTableCheckBloodPatient();
  loadTableCheckResultBloodPatient();
  if (bloodQueueConfirmState) loadTableCrossmatch();
  if (bloodQueueUsedWaitState) loadTableBloodQueueWaitUsed();
  if (bloodQueueUsedReadyState) loadTableBloodQueueUsedReady();
