...
function isIDcardPassport() {
  ...
  if (id == 1) {
    ...
    if ($("#lname").val() == "ชาวต่างชาติ") $("#lname").val("");
  } else if (id == 2) {
    document.getElementById("donoridcard").required = false;
...
function setGenderPrefix() {
  var genderid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
...
function setPrefixSelect() {
  var genderid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  var isIDcardPassport = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "1";
  $('.prefixid').select2({
    allowClear: true,
    width: "100%",
    theme: "bootstrap",
    ...
    if (e.params.args.data.item.isforeign == 1) {
      $("#lname").val("ชาวต่างชาติ"); // isIDcardPassport(2);

      document.getElementById("isidcardpassport2").checked = true;
    } else {
      if ($("#lname").val() == 'ชาวต่างชาติ') $("#lname").val(""); // isIDcardPassport(1);
      ...
      success: function success(data) {
        $.ajax({
          ...
          complete: function complete() {
            ...
            setTimeout(function () {// spinnerhide();
            }, delayInMilliseconds);
          },
          success: function success(data) {
            if (data.data.length > 0) {
              ...
              if (document.getElementById('donation_get_type_id3').checked) setBloodDonorRecord(data.data[0]);
            } else {
              var _swal;
...
function setBloodDonorRecord(data) {
  ...
  getAge({
    value: getDMY(data.patientanbirthday)
  });
...
function donationStatus(id) {
  ...
  if (id == 2) {
    ...
    $("#physical_status_1").prop('required', false); // $("#expired_date").datepicker("destroy");

    $("#expired_date").val("");
    $('#blood_group').prop('required', false); // $('#rh').prop('required',false)
    ...
    if (document.getElementById("donateid").value != "") {
      ...
      $("#expired_date").val("");
      $('#blood_group').prop('required', true); // $('#rh').prop('required',true)
...
function getAgeMsg(age) {
  if (!AgeFirst) if (age >= 70) {
    ...
    swal((_swal3 = {
      ...
      if (inputValue) {
        document.getElementById("donation_status2").checked = true; // $("#donatenocauseid").val(55);
...
function setDetailDonation() {// document.getElementById("bag_type_id").required = (donatenocauseid == 1);
  ...
  var donatenocauseid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
}
...
function setSDPShow() {
  ...
  if (id == 2) {
    for (var i = 0; i < op.length; i++) {
      op[i].value == "8" || op[i].value == "10" ? op[i].disabled = false : op[i].disabled = true;
    } // $('#bag_type_id').val(8);
    ...
    for (var i = 0; i < op.length; i++) {
      op[i].value == "9" ? op[i].disabled = false : op[i].disabled = true;
    }
...
function setPrint() {
  var donorid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  var donateid = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";
  var sdpstatus = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
function setPrintAutologousDirect() {
  var donation_get_type_id = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
...
function setDonateStatusDate() {// var state = document.getElementById("donatenostatusid").value;
  // if (state == 1) {
...
function loadTable() {
  var stateDonate = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (document.getElementById("btnAddModal")) document.getElementById("btnAddModal").hidden = false;
      var event_data = '';
      ...
      $.each(obj, function (index, value) {
        ...
        var colorNoStatus = "";
        var textNoStatus = '';
        var textSDPNoStatus = ''; // if (document.getElementById("btnAddModal"))
        ...
        if (posNeg(value) == 'Positive') {
          ...
          if (document.getElementById("btnAddModal")) document.getElementById("btnAddModal").hidden = true;
        ...
        if (value.sdpdonatenostatusid == 1) {
          if (document.getElementById("donation_type_id2")) document.getElementById("donation_type_id2").disabled = true;
          ...
          if (value.isconfirmdonorsdp == 1) {
            if (document.getElementById("donation_type_id2")) document.getElementById("donation_type_id2").disabled = false;
        ...
        event_data += '<td style="text-align: left; color:blue; font-weight: bold; font-size: 16px;" class="' + colorNoStatus + '" >'; // if(value.blood_invest_tube_edta == '' && value.sdpisdonateremark == ''){
        ...
        event_data += textNoStatus + textSDPNoStatus + (value.isdonateremark == 1 ? value.donateremark + '<br/>' : '') + ' ' + (value.isblood_invest_remark == 1 ? value.blood_invest_remark + '<br/>' : '') + ' ' + (value.iscross_remark == 1 ? 'หมายเหตุเลือดติดเชื้อ : ' + value.cross_remark + '<br/>' : '') + ' ' + (value.donationproblemsid > 0 ? 'ปัญหาของการรับบริจาค : ' + value.donationproblemsname + '<br/>' : '') + ' ' + (value.donatereactionid > 0 ? 'ปฏิกิริยาหลังเจาะ : ' + value.donatereactionname + '<br/>' : '') + ' ' + "<font color='#00008B'><b>" + (value.istube == 1 ? "เจาะ tube <br>" : '') + (value.blood_invest_tube_edta != '' ? "EDTA :" + value.blood_invest_tube_edta : '') + (value.blood_invest_tube_clotted != '' ? ",Clotted :" + value.blood_invest_tube_clotted : '') + (value.blood_invest_tube_acd != '' ? ",ACD :" + value.blood_invest_tube_acd : '') + "</b>" + (value.donation_status == 2 ? value.donatenocausename : '') + "</b></font>" + (value.sdpisdonateremark == 1 ? ",SDP Problem:" + value.sdpdonatenocauseremark : '');
      ...
      if (stateDonate && obj[0].isconfirmdonorsdp != 1) if (document.getElementById("btnAddModal")) document.getElementById("btnAddModal").hidden = true;
...
function confirmAddBloodDonation() {
  ...
  if (sdp30alertstatus) {
    swal({
      ...
      html: true,
      text: '<h3>อนุญาตให้บริจาคได้หรือไม่</h3>' + '<select id="warrantorsdp30"></select>',
      type: "input",
      showCancelButton: true,
...
function confirmAddBloodDonationandcheck() {
  if (valueNoStatus) {
    swal({
      ...
      html: true,
      text: '<h1>ยืนยันรับบริจาค</h1>' + '<input  type="text" autocomplete="off" value="" class="form-control" id="confirmusername" name="confirmusername" placeholder="Username">' + '<input style="margin-top:10px;" type="password" autocomplete="off" value="" class="form-control" id="confirmpassword" name="confirmpassword" placeholder="Password">',
      ...
      if (inputValue) {
        ...
        spinnershow();
        $.ajax({
          url: 'data/checklogin.php?confirmusername=' + confirmusername + '&confirmpassword=' + confirmpassword,
          dataType: 'json',
          type: 'get',
          complete: function complete() {
            var delayInMilliseconds = 200;
...
function loadTableChangeBloodGroup() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_chanhe_Blood").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function loadTableChangeName() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_chanhe_name").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function loadTableChangePassport() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_chanhe_passport").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function loadTableGetCard() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_chanhe_get_card").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function posNeg(v) {
  if (v.tpharpr == '+' || v.hbsag == '+' || v.hivagab == '+' || v.hcvab == '+' || v.hbvdna == '+' || v.hcvrna == '+' || v.hivrna == '+') {
    return 'Positive';
  } else if (v.tpharpr == '-' && v.hbsag == '-' && v.hivagab == '-' && v.hcvab == '-' && v.hbvdna == '-' && v.hcvrna == '-' && v.hivrna == '-') {
    return 'Negative';
  } else {
    return '';
  }
...
function setPosInfectious(v) {
  var str = "";
  if (v.tpharpr == '+') str += "SYPHILIS TPHA RPR(" + v.tpharpr_grade + "+),";
  if (v.hbsag == '+') str += "HBs Ag(" + v.hbsag_grade + "+),";
  if (v.hivagab == '+') str += "HIV Ag/Ab(" + v.hivagab_grade + "+),";
  if (v.hcvab == '+') str += "HCV Ab(" + v.hcvab_grade + "+),";
  if (v.hbvdna == '+') str += "HBV DNA(" + v.hbvdna_grade + "+),";
  if (v.hcvrna == '+') str += "HCV RNA(" + v.hcvrna_grade + "+),";
  if (v.hivrna == '+') str += "HIV RNA(" + v.hivrna_grade + "+),";
...
function loadTableLost() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_Lost").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function loadTableCheckBlood() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $("#list_table_json_check_blood").append(event_data); // $("#list_table_json").append(event_data);
    },
    error: function error(d) {
      /*console.log("error");*/
      alert("404. Please wait until the File is Loaded.");
...
function bagNumber() {
  ...
  $('#bag_number').val(bag_number_new); // cut = bag_number.substr(7,1)
  // console.log(chk)
...
function findBagNumber(barcode) {
  spinnershow();
  $.ajax({
    url: 'data/blood/blooddonorlist.php?barcode=' + barcode + '&activepage=1' + '&numrows=' + 25,
    dataType: 'json',
    type: 'get',
    complete: function complete() {
    ...
    success: function success(data) {
      ...
      if (val.length > 0) {
        swal({
          ...
          if (!inputValue) {
            window.location.href = 'blood-donor-record.php?id=' + val[0].donateid; // console.log(inputValue);
          }
...
setTimeout(function () {
  if (unitnameid == "") setDataModalSelectValue("unitnameid", localStorage.getItem("unitnameid"), localStorage.getItem("unitnamename"));
  if (activityid == "") setDataModalSelectValue("activityid", localStorage.getItem("activityid"), localStorage.getItem("activityname"));
}, 2000);
...
function showAntiPhone() {
  ...
  elem += '<th class="table-td-header" >Antibody<br/>Sceening</th>';
