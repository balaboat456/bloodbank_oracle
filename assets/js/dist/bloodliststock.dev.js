...
function loadTable() {
  ...
  var rfidcode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
  spinnershow();
  var keyword = '';
  var fromdate = '';
  var todate = '';
  ...
  $.ajax({
    url: 'data/blood/bloodstocklist.php?keyword=' + keyword + '&fromdate=' + fromdate + '&todate=' + todate + '&receivingtypeid2=' + receivingtypeid2 + '&bloodstocktypecross=' + bloodstocktypecross + '&donatebloodtype=' + donatebloodtype + '&bag_number=' + bag_number + '&rfidcode=' + rfidcode + '&activepage=' + active + '&hn=' + hn + '&numrows=' + 25,
    dataType: 'json',
    type: 'get',
    ...
    success: function success(data) {
      ...
      $.each(obj, function (index, value) {
        ...
        event_data += '<tr>';
        if (value.seq == '1') event_data += '<td rowspan="' + value.bloodstockmainamount + '" style="width:40px">' + num + '</td>';
        if (value.seq == '1') event_data += '<td rowspan="' + value.bloodstockmainamount + '" >' + getDMY2(value.bloodstockmaindate) + ' ' + value.bloodstockmaintime + '</td>';
        if (value.seq == '1') event_data += '<td rowspan="' + value.bloodstockmainamount + '">' + value.receivingtypename + '</td>';
        ...
        if (value.seq == '1') event_data += '<td rowspan="' + value.bloodstockmainamount + '" style="width:200px" id="staffname' + index + '">' + value.bloodstockcreatefullname + '</td>';
        var onclickblood = 'getdatastaffname(' + index + '); addBloodBlank2(' + index + '); ';
        if (value.receivingtypeid == '10') onclickblood = 'getdatastaffname(' + index + '); addBloodBlank2(' + index + '); ';
