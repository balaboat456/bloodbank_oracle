...
$(document).ready(function () {
  $("#confirmblooddonationid").select2({
    width: "100%",
    theme: "bootstrap"
  });
  ...
  $("#blood_group").select2({
    width: "100%",
    theme: "bootstrap",
    color: "red" // templateResult: function (state) 
    // { 
    //     if (!state.id) {
    //         return state.text;
    //     }
  ...
  $("#rh").select2({
    width: "100%",
    theme: "bootstrap" // templateResult: function (state) 
    //     { 
    //         if (!state.id) {
    //             return state.text;
    //         }
  ...
  $('#search').select2({
    ...
    getAge({
      value: document.getElementById("donorbirthday").value
    });
  ...
  function alertSceenPost(item) {
    var item2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    var screenpoststatus = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  */
  ...
  function setItem(item) {
    ...
    document.getElementById("zipcode_current").value = item.zipcode_current; // document.getElementById("souvenirid").value = item.souvenirid;
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        if (data.data.length > 0) {
          ...
          setBtnChangeName();
          setBtnChangePassport();
          setBtnChangeBlood_Group(); // $('#bag_staff_id').prop('required', true)
  ...
  function selectDonor() {
    ...
    var b = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "";
    var item = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
    var swalType = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 'warning';
    var bloodtype = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : "";
    swal({
  ...
  $('.staffsouvenirid').select2({
    width: "180px",
    theme: "bootstrap" // placeholder: "เลือกเจ้าหน้าที่ผู้จ่ายเข็ม",

  });
  $('.staffcardid').select2({
    width: "100%",
    theme: "bootstrap" // placeholder: "เจ้าหน้าที่ผู้จ่ายบัตร",
  ...
  $('.ischecklab').select2({
    width: "100%",
    theme: "bootstrap" // placeholder: "เจ้าหน้าที่ตรวจสอบผล Lab",

  });
  $('.issdpsave').select2({
    width: "100%",
    theme: "bootstrap" // placeholder: "ผู้บันทึก",
  ...
  setCheckMarkAutologousAppointment(); // setProvince();
  // setDistrict();
  // setSubDistrict();
  ...
  function setCountryCurrent() {
    // Set Default Country
    if (!countrycurrentid) countrycurrentid = 'THA';
  ...
  $('#myform').submit(function () {
    ...
    return false; // if (document.getElementById("isidcardpassport1").checked && $("#notdonoridcard").val() == "")
  ...
  if (donateid == "") {
    setInterval(function () {
      if (IntervalState) loadCardReader();
    }, 1000);
  }
  ...
  function loadCardReader() {
    $.ajax({
      ...
      success: function success(data) {
        ...
        var objRaw = JSON.stringify(data); // var objj = JSON.parse(JSON.stringify(data).replace(/null/g, '""'));

        console.log("=====success=====" + obj.success);

        if (obj.success) {
          IntervalState = false; // console.log(obj)
          ...
          if (!smartCardStatus) {
            ...
            $('#curr_county').prop('hidden', true); // $("#address_street_current").prop('required',true);
            ...
            $.ajax({
              ...
              data: {
                donoridcard: obj.data.citizenid,
                donatelogtext: objRaw
              },
              type: 'POST',
              dataType: 'json',
            ...
            $.ajax({
              ...
              success: function success(obj) {
                ...
                pro = data.data.address2.province.slice(7); // console.log(data.data.address2.amphur.substr(0,3))
                ...
                console.log('new');
                console.log(new_str);
                if (olddatalengthstatus) if (new_str != oldstrtrim) {
                  swal({
            ...
            document.getElementById("donorbirthday").readOnly = true; // $( "#donorbirthday" ).datepicker( "hidden" )

            $("#donorbirthday").datepicker("destroy");
            document.getElementById("donorage").readOnly = true;

            if (!smartCardStatus) {
              $('#add_curr').prop('hidden', false); // $('#add_curr_st').prop('hidden',false);

              $('#curr_zip').prop('hidden', false);
              $('#curr_dis').prop('hidden', false);
              $('#curr_county').prop('hidden', false); // $("#address_street_current").prop('required',true);
              ...
              $("#address_current").prop('required', false); // $("#address_street_current").prop('required',false);
            }
          }
        } else {
  ...
  function loadDonorData(objData) {
    $.ajax({
      ...
      type: 'get',
      success: function success(data) {
        if (data.status) if (data.data.length > 0) {
  ...
  function setCardReaderData(obj) {
    ...
    $.ajax({
      ...
      data: {
        prefixname: obj.data.prefixname_th,
        genderid: obj.data.gender
      },
      type: 'POST',
      dataType: 'json',
      success: function success(obj) {
...
function findRold() {
  var addressData = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  var result = "";
  var obj = addressData.split(" ");
  if (obj.length > 0) $.each(obj, function (index, value) {
...
function setDataModalSelectValue2() {
  var elem = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var itemid = arguments.length > 1 ? arguments[1] : undefined;
  var itemtext = arguments.length > 2 ? arguments[2] : undefined;
  var select = $('.' + elem);
