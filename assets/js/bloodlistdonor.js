...
function loadTable(active = 1) {
    ...
    $.ajax({
        url: 'data/blood/blooddonorlist.php?keyword=' + keyword +
            '&fromdate=' + fromdate +
            '&todate=' + todate +
        ...
        success: function(data) {
            ...
            $.each(data.data, function(index, value) {
                ...
                if (!isNaN(NumBag_Number1))
                    if (value.bag_number.substring(7, 8) == 0)
                        bagnumberArray1.push(NumBag_Number1);

                var NumBag_Number2 = parseInt(value.bag_number.split(".").join(""));
                if (!isNaN(NumBag_Number2))
                    if (value.bag_number.substring(7, 8) == 1)
                        bagnumberArray2.push(NumBag_Number2);
...
function setLoopNumber(bagnumberfrom, bagnumberto, Min, Max, rowgroup, rowname = "") {
    try {
        ...
        */
        ...
        var bagnumberArrayObj = bagnumberArray1 ;
        if(rowgroup == 2)
        {
            bagnumberArrayObj = bagnumberArray2 ;
        }

        for (let i = NumBag_NumberFrom; i <= NumBag_NumberTo; i++) {
            var status = true;
            var bag_number = i;
            $.each(bagnumberArrayObj, function(index, value) {
                if (bag_number == value) {
                    status = false;
                }
            });
