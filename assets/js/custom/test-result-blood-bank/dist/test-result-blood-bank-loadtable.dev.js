...
function loadCheckRequestTab2() {
  ...
  var hn = document.getElementById("hn").value;
  $.ajax({
    url: 'data/testresultbloodbank/test-result-blood-bank-list.php?hn=' + hn + '&fromdate=' + fromdate + '&todate=' + todate,
    dataType: 'json',
    type: 'get',
    success: function success(data) {
...
function loadCheckRequestSingle(id) {
  $.ajax({
    url: 'data/testresultbloodbank/test-result-blood-bank-list.php?id=' + id,
    dataType: 'json',
    type: 'get',
    success: function success(data) {
      console.log(data);
      item_lab_new = data.data[0];
      setValueItem();
    },
    error: function error(d) {
      /*console.log("error");*/
...
function loadCheckRequestTab2_item2(id) {
  $.ajax({
    ...
    success: function success(data) {
      ...
      var obj = JSON.parse(JSON.stringify(data.data).replace(/null/g, '""').replace(/"\"\""/g, '""')); // count = obj.length ;
...
function loadLabForm() {
  var labformtypeid = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "1";
  item_lab_form = '';
  $.ajax({
