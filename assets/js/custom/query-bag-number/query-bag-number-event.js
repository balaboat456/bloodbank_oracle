...
function getStatus(value) {
    ...
    if(value.bloodstockstatusid == 8)
    {
        status = value.bloodremarktext;
    }else if (value.bloodstockstatusid == 1 || value.bloodstockstatusid == 6) {
        console.log("====1======");
    ...
    } else if (value.bloodstockpaytypeid != "" && value.bloodstockpaytypeid != "0") {
        console.log("====4======");
        status = value.bloodstockpaytypename2;

        if(value.bloodstockpaytypeid == 9)
        {
            if(value.bloodbrokenname != '')
            {
                status += ' ('+value.bloodbrokenname+')';
            }else
            {
                status += ' (ไม่ระบุสาเหตุ)';
            }
...
function loadTableBagNumber(bag_number = '', sub = '', bloodtype = '') {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // $("#blood-query-history").text(data.total);
            var obj = JSON.parse(JSON.stringify(data.data).replace(/null/g, '""').replace(/"\"\""/g, '""'));
...
function chActiveRow(id, self, bag_number = '', sub = '', bloodtype = '') {
