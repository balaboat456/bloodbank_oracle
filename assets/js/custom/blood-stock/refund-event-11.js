    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(obj, function(index, value) {
                ...
                event_data += '<td >' + (value.sum_qty - value.sum_payqty) + ((parseInt(value.sum_qty - value.sum_payqty) < 0)?'<br/><p style="color:red">รับเกินจำนวน</p>':'' ) + '</td>';
                event_data += '</tr>';
...
function confirmchReceivingTypeid_11() {
    var item = dataRefundReceivingTypeid_11;
    ...
    $("#outhospitalid").val(item.hospitalid);
