...
function loadTableBloodBorrowReceivingtypeid5() {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      $.each(obj, function (index, value) {
        ...
        event_data += '<td >' + value.group_bloodstockmaindate + '</td>'; // event_data += '<td >' + (value.sum_qty - value.net_sum_bloodgroup_get)   + '</td>';

        event_data += '</tr>';
        countReceivingTypeid_5++;
      });
      $("#list_blood_borrow_receivingtypeid_5").append(event_data); // console.log(data)
    },
...
function chActiveReceivingTypeid_5(self) {
  ...
  chk_bag_icon++; // console.log(dataReceivingTypeid_5);
}
