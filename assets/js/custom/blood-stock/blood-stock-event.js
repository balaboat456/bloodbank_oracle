...
function loadTableBloodStockPay(receivingtypeid = "") {

    dataBloodStockPay = [];
    $("#bloodborrowrefunditemid").val("");
    var hospitalid = $("#hospital").val();

    // document.getElementById('label_stock_pay').innerHTML   = "ข้อมูลประวัติการแลกเลือดกับ"+outhospitalname;
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(obj, function(index, value) {
                ...
                event_data += '<td>' + value.sum_group_bloodgroup2 + ((parseInt(value.sum_group_bloodgroup2) < 0)?'<br/><p style="color:red">รับเกินจำนวน</p>':'' ) + '</td>';
