...
function loadTable() {
  ...
  $.ajax({
    url: 'data/serumtear/serum-tear.php?hn=' + hn,
    dataType: 'json',
    type: 'get',
    success: function success(data) {
      var body = document.getElementById("list_table_serum_tear_letting").getElementsByTagName('tbody')[0];
      ...
      $.each(obj, function (index, value) {
        ...
        event_data += '<td>' + value.report + '</td>';
        event_data += '<td>' + value.usercreate + '</td>';
        event_data += '</tr>';
      });
      $("#list_table_serum_tear_letting").append(event_data);
