    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(obj, function(index, value) {
                ...
                } else if (value.ispaybloodstatus == 1 && (!(value.requestbloodstatusid == 3 || value.requestbloodstatusid == 4)))
                ...
                if (value.requestbloodstatusid == 4 || value.requestbloodstatusid == 3) {
                    ...
                    event_data += '<button type="button" onclick="selectTableReq(this); setEnableDisable(false);"  class="btn btn-success m-l-5">';
                    event_data += '<span class="btn-label"><i class="fa fa-search"></i></span>ดูข้อมูล';
                    event_data += '</button>&nbsp;&nbsp;';
                }
                event_data += '</td>';
...
function addListRequestBlood_3(id = '') {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#list_request_blood_modal_3").append(event_data);

            // document.getElementById("trbloodReq30").click();
        },
        error: function(d) {
            /*console.log("error");*/
            alert("404. Please wait until the File is Loaded.");
...
function addListRequestBlood_4(id = '', bloodtype = '') {
    $.ajax({
...
function setInsertReq() {
    ...
    if (localStorage.getItem("pointwardid", ''))
        setDataModalSelectValue('requestunit', localStorage.getItem("pointwardid", ''), localStorage.getItem("pointwardname", ''));

    $('#usedunit').val(null).empty();
    $('#bloodnotificationtypeid').val(null).empty();
    $('#nurseid').val(null).empty();
    // $('#doctorid').val(null).empty();
    $('#blood_driller_id').val(null).empty();
    $('#blood_certifier_id').val(null).empty();
    $('#diseasegroupid').val(null).empty();
    $('#diagnosisid').val(null).empty();

    // setDataModalSelectValue('bloodgroupid',null,'');
...
function select_date_wash(date) {
    var val = date.split("-");

    var val2 = val[2] + '/' + parseInt(val[1]) + '/' + parseInt(val[0]);
...
function setTableReq(data, reorder = false, sta_order) {
    console.log(data);

    $('#bloodwash_id1').val(data.bloodwash_id1);
    $('#bloodwash_id2').val(data.bloodwash_id2);
    $('#bloodwash_id3').val(data.bloodwash_id3);
    $('#bloodwash_id4').val(data.bloodwash_id4);

    if (data.bloodwash_date1 != '') {
        $('#bloodwash_date1').append("<option value=" + data.bloodwash_date1 + ">" + select_date_wash(data.bloodwash_date1) + "</option>")
        $('#bloodwash_date1').val(data.bloodwash_date1);
        ldate++;
    }
    if (data.bloodwash_date2 != '') {
        $('#bloodwash_date2').append("<option value=" + data.bloodwash_date2 + ">" + select_date_wash(data.bloodwash_date2) + "</option>")
        $('#bloodwash_date2').val(data.bloodwash_date2);
        ldate++;
    }

    if (data.bloodwash_date3 != '') {
        $('#bloodwash_date3').append("<option value=" + data.bloodwash_date3 + ">" + select_date_wash(data.bloodwash_date3) + "</option>")
        $('#bloodwash_date3').val(data.bloodwash_date3);
        ldate++;
    }
    if (data.bloodwash_date4 != '') {
        $('#bloodwash_date4').append("<option value=" + data.bloodwash_date4 + ">" + select_date_wash(data.bloodwash_date4) + "</option>")
        $('#bloodwash_date4').val(data.bloodwash_date4);
        ldate++;
    }


    if (data.bloodwash_round1 != '') {
        document.getElementById("bloodwash_round1_3").checked = true;
    }
    if (data.bloodwash_round2 != '') {
        document.getElementById("bloodwash_round2_3").checked = true;
    }
    if (data.bloodwash_round3 != '') {
        document.getElementById("bloodwash_round3_3").checked = true;
    }
    if (data.bloodwash_round4 != '') {
        document.getElementById("bloodwash_round4_3").checked = true;
    }
    $('#bloodwash_unit1').val(data.bloodwash_unit1);
    $('#bloodwash_unit2').val(data.bloodwash_unit2);
    $('#bloodwash_unit3').val(data.bloodwash_unit3);
    $('#bloodwash_unit4').val(data.bloodwash_unit4);

    $('#bloodwash_ml1').val(data.bloodwash_ml1);
    $('#bloodwash_ml2').val(data.bloodwash_ml2);
    $('#bloodwash_ml3').val(data.bloodwash_ml3);
    $('#bloodwash_ml4').val(data.bloodwash_ml4);

    $('#bloodwash_intense1').val(data.bloodwash_intense1);
    $('#bloodwash_intense2').val(data.bloodwash_intense2);
    $('#bloodwash_intense3').val(data.bloodwash_intense3);
    $('#bloodwash_intense4').val(data.bloodwash_intense4);

    $('#bloodwash_user').val(data.bloodwash_user);
    $('#bloodwash_tel').val(data.bloodwash_tel);
    ...
    $('#bloodgrouppregnant').val(data.bloodgrouppregnant);
    ...
    // document.getElementById("forchildren").disabled = (!(data.requestunit == 191));
    ...
    $("#group_antibody").val(data.group_antibody);
...
function getTableTypeOfRequestBlood() {
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        if (rows[i].cells[2].children[0].value == 'LPRC' ||
            rows[i].cells[2].children[0].value == 'LPRC-O' ||
            rows[i].cells[2].children[0].value == 'LDPRC' ||
            rows[i].cells[2].children[0].value == 'PRC' ||
            rows[i].cells[2].children[0].value == 'SDR' ||
            rows[i].cells[2].children[0].value == 'LDSDR'
        )
            rows[i].style.backgroundColor = "#F39";
        else if (rows[i].cells[2].children[0].value == 'PC' ||
            rows[i].cells[2].children[0].value == 'LPPC' ||
            rows[i].cells[2].children[0].value == 'SDP' ||
            rows[i].cells[2].children[0].value == 'SDP_PAS' ||
            rows[i].cells[2].children[0].value == 'LDPPC' ||
            rows[i].cells[2].children[0].value == 'LDPPC_PAS' ||
            rows[i].cells[2].children[0].value == 'LPPC_PAS'
        )
            rows[i].style.backgroundColor = "#FFFF00";
        else if (rows[i].cells[2].children[0].value == 'FFP' ||
            rows[i].cells[2].children[0].value == 'CRP'
        )
            rows[i].style.backgroundColor = "#6C0";
        else if (rows[i].cells[2].children[0].value == 'CRYO' || rows[i].cells[2].children[0].value == 'HTFDC')
...
function setEnableDisable(status = false) {
    ...
    document.getElementById('bloodgrouppregnant').disabled = status;
    ...
    document.getElementById('btnAddRowReqTab1').disabled = status;

    // document.getElementById("btnTab2").style.display = (status)?"none":"block";
    // document.getElementById("btnTab3").style.display = (status)?"none":"block";

}

function setnew_request() {
    $('#requestbloodstatusid').val('');
}
...
function addRowReq2(im = [], bloodnotificationtypeid2 = '') {
