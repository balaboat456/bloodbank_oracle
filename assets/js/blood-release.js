function loadTable() {
    ...
    $.ajax({
        url: 'data/blood/blood-release.php?hn=' + hn +
...
function findTableBagNumber(number) {
    ...
    for (var i = 1; i < rows.length; i++) {
        if (rows[i].cells[12].innerHTML == number) {
            result = true;
            rows[i].cells[16].getElementsByTagName('input')[0].checked = true;
            rows[i].cells[16].getElementsByTagName('input')[0].focus();
            var row = rows[i];
...
function findTableRFID(number) {
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        $.each(numberArray, function(index, value) {
            ...
            if (value != "") {
                if (rows[i].cells[14].innerHTML == value) {
                    result = true;
                    rows[i].cells[16].getElementsByTagName('input')[0].checked = true;
                    rows[i].cells[16].getElementsByTagName('input')[0].focus();
                    var row = rows[i];
