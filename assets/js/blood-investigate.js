...
function chActive(id = 0) {
    ...
    if (document.getElementById("ckblood" + id))
        document.getElementById("ckblood" + id).checked = true;
    // alert(count);
    for (i = 0; i < count; i++) {
    ...
    document.getElementById('bloodrhsceen_cross_c').value = datablood[id].bloodrhsceen_cross_c;
    // document.getElementById('bloodrhsceen_cross').value = datablood[id].bloodrhsceen_cross;
    ...
    document.getElementById('barcodeantibody').value = datablood[id].bag_number;
    ...
    $("#bloodrhsceencross").text(datablood[id].rhname_cross3_2);

    // document.getElementById('tubeantibodyscreeningtest').value = datablood[id].tubeantibodyscreeningtest;
...
$('#myform').submit(function(e) {
    ...
    var staffantibodyscreeningtest = document.getElementById("staffantibodyscreeningtest").value;
    // var tubeantibodyscreeningtest = document.getElementById("tubeantibodyscreeningtest").checked;
        ...
        while (cell = row.cells[c++]) {
            if (c == 1) {
                if (cell.children[0].value != "undefined") {
                    arrCell = realMerge(arrCell, [{ donateantisceentest: cell.children[0].value }]);
                } else {
                    arrCell = realMerge(arrCell, [{ donateantisceentest: "" }]);
                }
            }

            if (c == 2)
                arrCell = realMerge(arrCell, [{ donateantisceenp1mi: cell.children[0].value }]);

            if (c == 3)
                arrCell = realMerge(arrCell, [{ donateantisceeno: cell.children[0].value }]);

            if (c == 4)
                arrCell = realMerge(arrCell, [{ donateantisceeno1: cell.children[0].value }]);

            if (c == 5)
                arrCell = realMerge(arrCell, [{ donateantisceeno2: cell.children[0].value }]);

            if (c == 6) {
                arrCell = realMerge(arrCell, [{ donateantisceenlotno: cell.children[0].value }]);
            }
...
function findAntibodyIdenTest(table) {
    ...
    while (row = table.rows[r++]) {
        ...
        while (cell = row.cells[c++]) {
            if (c == 1)
                arrCell = realMerge(arrCell, [{ donateantiidentest: cell.children[0].value }]);

            if (c == 2)
                arrCell = realMerge(arrCell, [{ donateantiiden1: cell.children[0].value }]);

            if (c == 3)
                arrCell = realMerge(arrCell, [{ donateantiiden2: cell.children[0].value }]);

            if (c == 4)
                arrCell = realMerge(arrCell, [{ donateantiiden3: cell.children[0].value }]);

            if (c == 5)
                arrCell = realMerge(arrCell, [{ donateantiiden4: cell.children[0].value }]);

            if (c == 6)
                arrCell = realMerge(arrCell, [{ donateantiiden5: cell.children[0].value }]);

            if (c == 7)
                arrCell = realMerge(arrCell, [{ donateantiiden6: cell.children[0].value }]);

            if (c == 8)
                arrCell = realMerge(arrCell, [{ donateantiiden7: cell.children[0].value }]);

            if (c == 9)
                arrCell = realMerge(arrCell, [{ donateantiiden8: cell.children[0].value }]);

            if (c == 10)
                arrCell = realMerge(arrCell, [{ donateantiiden9: cell.children[0].value }]);

            if (c == 11)
                arrCell = realMerge(arrCell, [{ donateantiiden10: cell.children[0].value }]);

            if (c == 12)
                arrCell = realMerge(arrCell, [{ donateantiiden11: cell.children[0].value }]);

            if (c == 13)
                arrCell = realMerge(arrCell, [{ donateantiidenauto: cell.children[0].value }]);

            if (c == 14)
                arrCell = realMerge(arrCell, [{ donateantiidenlotno: cell.children[0].value }]);
...
function findRh(table) {
    ...
    while (row = table.rows[r++]) {
        ...
        while (cell = row.cells[c++]) {
            if (c == 1)
                arrCell = realMerge(arrCell, [{ donaterhreagent: cell.children[0].value }]);

            if (c == 2)
                arrCell = realMerge(arrCell, [{ donaterhrt: cell.children[0].value }]);

            if (c == 3)
                arrCell = realMerge(arrCell, [{ donaterh37c: cell.children[0].value }]);

            if (c == 4)
                arrCell = realMerge(arrCell, [{ donaterhiat: cell.children[0].value }]);

            if (c == 5)
                arrCell = realMerge(arrCell, [{ donaterhccc: cell.children[0].value }]);

            if (c == 6)
                arrCell = realMerge(arrCell, [{ donaterhresult: cell.children[0].value }]);
...
function findABO(table) {
    ...
    while (row = table.rows[r++]) {
        ...
        while (cell = row.cells[c++]) {
            ...
            if (c == 1)
                arrCell = realMerge(arrCell, [{ donatereagent: cell.children[0].value }]);

            if (c == 2)
                arrCell = realMerge(arrCell, [{ donateantia: cell.children[0].value }]);

            if (c == 3)
                arrCell = realMerge(arrCell, [{ donateantib: cell.children[0].value }]);

            if (c == 4)
                arrCell = realMerge(arrCell, [{ donateantiab: cell.children[0].value }]);

            if (c == 5)
                arrCell = realMerge(arrCell, [{ donateacells: cell.children[0].value }]);

            if (c == 6)
                arrCell = realMerge(arrCell, [{ donatebcells: cell.children[0].value }]);

            if (c == 7)
                arrCell = realMerge(arrCell, [{ donateocells: cell.children[0].value }]);

            if (c == 8)
                arrCell = realMerge(arrCell, [{ donatebloodgroup: cell.children[0].value }]);
...
function findABOModal(table) {
    ...
    while (row = table.rows[r++]) {
        ...
        while (cell = row.cells[c++]) {
            ...
            if (c == 1)
                arrCell = realMerge(arrCell, [{ donatemodalreagent: cell.children[0].value }]);

            if (c == 2)
                arrCell = realMerge(arrCell, [{ donatemodalantia: cell.children[0].value }]);

            if (c == 3)
                arrCell = realMerge(arrCell, [{ donatemodalantib: cell.children[0].value }]);

            if (c == 4)
                arrCell = realMerge(arrCell, [{ donatemodalantiab: cell.children[0].value }]);

            if (c == 5)
                arrCell = realMerge(arrCell, [{ donatemodalacells: cell.children[0].value }]);

            if (c == 6)
                arrCell = realMerge(arrCell, [{ donatemodalbcells: cell.children[0].value }]);

            if (c == 7)
                arrCell = realMerge(arrCell, [{ donatemodalocells: cell.children[0].value }]);

            if (c == 8)
                arrCell = realMerge(arrCell, [{ donatemodalbloodgroup: cell.children[0].value }]);

            if (c == 9)
                arrCell = realMerge(arrCell, [{ donatemodalaboremark: cell.innerHTML }]);

            if (c == 10)
                arrCell = realMerge(arrCell, [{ donatemodalstaff: cell.innerHTML }]);

            if (c == 11)
                arrCell = realMerge(arrCell, [{ donatemodaldatetime: cell.innerHTML }]);
...
function antibodyModal() {
    ...
    console.log(document.getElementById('phenotype').value);
}
