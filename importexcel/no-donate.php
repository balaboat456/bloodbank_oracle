...
//     $y = substr($m,strlen($m)-2);
    ...
    $inputFileName="not/2564/09-2564.xls";
    ...
    foreach ($namedDataArray as $resx) {
        ...
        $donateremark = $resx['หมายเหตุ']." ".$resx['สถานะการบริจาค'].$resx['ปฏิกิริยาหลังเจาะเก็บ'].$resx['ปฏิกิริยาหลังเจาะเก็บอื่น'].$resx['ปัญหาการรับบริจาค1'].$resx['ปัญหาการรับบริจาค2'].$resx['ปัญหาการรับบริจาค3'].$resx['ปัญหาการรับบริจาค4'].$resx['ปัญหาการรับบริจาค5'];
        $remarkaccident = "";
        $isdonateremark = "1";
        $isconfirmblooddonation = "1";

        $donatenocausename = $resx['สถานะการบริจาค'];
        $sql = "SELECT * FROM donate_no_cause WHERE donatenocausename = '$donatenocausename'";
        $result = mysqli_query($conn, $sql);
        $row = $result->fetch_assoc();
        $donatenocauseid = $row['donatenocauseid'];
