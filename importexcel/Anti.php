...
$status = true;
    ...
    $inputFileName="Phone_Anti/Anti-BB.xlsx";
    ...
    foreach ($namedDataArray as $resx) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $sql = "UPDATE donate SET donateantibody='$Anti'  WHERE bag_number = '$bag_number'";
            $result = mysqli_query($conn, $sql);
            if(empty($result))
            $status = 0;

            $sql = "UPDATE donor SET antibody='$Anti'  WHERE donorid = '$donorid'";
            $result = mysqli_query($conn, $sql);
            if(empty($result))
            $status = 0;
