...
$status = true;
    ...
    $inputFileName="request-blood/Crossmatch_15.xlsx";
    ...
    foreach ($namedDataArray as $resx) {
        ...
        $requestblooddate =  dmyToymd2($resx['bdvstdate']);
        $requestbloodtime =  getTime($resx['bdvsttime']);

        $seq =  $resx['itemno'];
        $bag_number =  substr($resx['dnrno'],0,3).".".substr($resx['dnrno'],3,2).".".substr($resx['dnrno'],5,1).".".substr($resx['dnrno'],6);
        $bloodtype =  $resx['abbrnm'];
        $bloodgroupid =  $resx['bloodgroup'];
        $rhid =  getRh2($resx['rh']);
        $cat =  getRh3($resx['cat']);
        $crossmacthstatusid =  getResult($resx['result']);
        $crossmacthresultid = getStatus($resx['status']);
        $doctorid =  $resx['dct'];
        $isbloodpreparation =  $resx['csmtstf'];
        $requestbloodcrossmacthdatetime =  dmyToymd2($resx['csmtdate']).' '.getTime($resx['csmttime']);
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $requestbloodid = $resultArray['requestbloodid'];
...
function getStatus($v)
{
    ...
    {
        $result = "1";
    }else if($v == "Most compatible")
    {
        $result = "2";
    }else if($v == "Uncrossmatch")
    {
        $result = "3";
    }else if($v == "A")
    {
