...
$curl = curl_init();
curl_setopt_array($curl, array(
    CURLOPT_URL => '192.168.1.115/PRODUCTION/API/MasterData/dct',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 0,
  ...
  curl_close($curl);
    ...
    foreach ($namedDataArray as $key=>$resx) {
        ...
        $dct =  $resx->dct;
        $dspname =  str_replace('   ',' ',$resx->dspname);
        $wrkdivnm =  $resx->wrkdivnm;

        $sql = "SELECT * FROM doctor WHERE doctorcode2 = '$dct'";
        ...
        {
            $sql = "UPDATE doctor
            SET  doctorname = '$dspname',
            doctorunit = '$wrkdivnm'
            WHERE doctorcode2 = '$dct'
            ";
