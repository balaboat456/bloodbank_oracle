...
include('jasper-connection.php');
 ...
 $volume = round($_GET['volume']);
 $bag_number = $_GET['bag_number'];
 $datestart = $_GET['datestart'];
 $dateexp = $_GET['dateexp'];
 $sdpprodyltyield = round($_GET['sdpprodyltyield'],2);
 $sdpprodunit = round($_GET['sdpprodunit'],2);

 $sdpresultvolume = $_GET['sdpresultvolume'];
...
$controls = array(
	...
	// "dose" => "$sdpresultvolume"
	"dose" => "1"
);

$report = $client->reportService()->runReport($jasper_server_config_path . 'blood_sticker_sdp', 'pdf',null,null,$controls);
